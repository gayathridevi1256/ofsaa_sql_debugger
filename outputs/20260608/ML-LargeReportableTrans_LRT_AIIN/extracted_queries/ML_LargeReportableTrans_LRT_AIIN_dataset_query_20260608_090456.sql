-- DATASET QUERY
-- Extracted: 2026-06-08 09:04:56.716018
...
select
(select cal.clndr_dt from fccmatomic.KDD_CAL cal where cal.CLNDR_NM = 'SYSCAL' and cal.CLNDR_DAY_AGE = 14 - 1) as Min_Dt,
...
(select add_days(cal.CLNDR_DT, -90) from fccmatomic.KDD_CAL cal where cal.CLNDR_NM = 'SYSCAL' and cal.CLNDR_DAY_AGE = 0) as Max_Days_Opened_Dt
FROM dual
),
...
(SELECT  
          ...
          (CASE WHEN tr.PASS_THRU_FL='Y' THEN DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM) ELSE 0 END) PASS_THRU_AMT,        
          (CASE WHEN DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM) - TRUNC(DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM), -4) = 0 THEN DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM) 
                 WHEN tr.RCV_TRXN_ACTVY_AM - TRUNC(tr.RCV_TRXN_ACTVY_AM, -4) = 0 THEN DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM)
                 WHEN tr.SEND_TRXN_ACTVY_AM - TRUNC(tr.SEND_TRXN_ACTVY_AM, -4) = 0 THEN DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM)
                 ELSE 0 END) LRF_AMT,
...
and ('Y' = 'Y' or   (tr.UNRLTD_PARTY_FL = 'Y' or tr.UNRLTD_PARTY_FL IS NULL))
and ('Y' = 'Y' or NOT(tr.BANK_TO_BANK_TRNFR_FL = 'Y' and tr.PASS_THRU_FL = 'N'))
...
(SELECT
          ...
          (CASE WHEN tr.PASS_THRU_FL='Y' THEN DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM) ELSE 0 END) PASS_THRU_AMT,        
          (CASE WHEN DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM) - TRUNC(DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM), -4) = 0 THEN DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM) 
                 WHEN tr.CLR_TRXN_ACTVY_AM - TRUNC(tr.CLR_TRXN_ACTVY_AM, -4) = 0 THEN DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM)
                 WHEN tr.DEP_TRXN_ACTVY_AM - TRUNC(tr.DEP_TRXN_ACTVY_AM, -4) = 0 THEN DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM)
                 WHEN tr.ISSUE_TRXN_ACTVY_AM - TRUNC(tr.ISSUE_TRXN_ACTVY_AM, -4) = 0 THEN DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM)
                 ELSE 0 END) LRF_AMT,
				  case when tr.TRSTD_TRXN_FL = 'Y' then DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM) else 0 end as Trusted_Trans_Amt,
          tr.FUNC_CRNCY_CD
...
WHERE
   ('Y'='Y' OR tr.SRC_SYS_CD IN ('Inactive'))
and tr.MANTAS_TRXN_PURP_CD = 'GENERAL'
and tr.MANTAS_TRXN_PRDCT_CD in ('CASH-EQ-CASHIER-CHECK', 'CASH-EQ-CERT-CHECK', 'CASH-EQ-MONEY-ORDER', 'CASH-EQ-TRAVELERS-CHECK', 'CASH-EQ-OTHER', 'CASH-LETTER', 'CHECK', 'PAPER-OTHER', 'CHECK-ACH')
and DECODE('B','F',tr.TRXN_FUNC_AM,tr.TRXN_BASE_AM) >=3000
--31536090 
--21153210
--and tr.MANTAS_POST_DT IN (SELECT KC.CLNDR_DT FROM fccmatomic.KDD_CAL KC WHERE KC.CLNDR_NM='SYSCAL' AND KC.CLNDR_DAY_AGE between 0 and 14 - 1)
and tr.DATA_DUMP_DT >= (select Min_Dt from clndr_vw) and tr.DATA_DUMP_DT <= (select Max_Dt from clndr_vw)
and ('Y' = 'Y' or   (tr.UNRLTD_PARTY_FL = 'Y' or tr.UNRLTD_PARTY_FL IS NULL))
and ('Y' = 'Y' or NOT(tr.BANK_TO_BANK_TRNFR_FL = 'Y' and tr.PASS_THRU_FL = 'N'))
and ('Y' = 'Y' or COALESCE(tr.TRSTD_TRXN_FL,'N') ='N')
...
where
 (( Overall_Risk = 'HR' and 
   ...
   (g.Tot_Trxn_Am_Cdt >= 250000 or g.Tot_Trxn_Am_Dbt >= 250000)
   )
    or
    (Overall_Risk = 'MR' and 
    SEASONED_CUST_FL = 1 and
       (g.Tot_Trxn_Am_Cdt >= 350000 or g.Tot_Trxn_Am_Dbt >= 350000))
    or
    ( Overall_Risk = 'RR'and 
    SEASONED_CUST_FL = 1 and
       (g.Tot_Trxn_Am_Cdt >= 250000 or g.Tot_Trxn_Am_Dbt >= 250000))
